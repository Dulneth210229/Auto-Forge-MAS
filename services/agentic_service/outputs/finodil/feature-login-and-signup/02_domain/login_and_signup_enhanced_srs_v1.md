# Enhanced Software Requirements Specification: Login and Signup
...
- **FR-DOM-001** **[DOMAIN ADDED]**: After a defined number of consecutive failed login attempts (5), the account should be temporarily locked for a cooldown period (15 minutes) or until the account owner resets their password, whichever comes first. — Priority: Must Have
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#0)
- **FR-DOM-002** **[DOMAIN ADDED]**: When a user requests a password reset, the system issues a single-use, time-limited reset token (expiring after 30 minutes) sent to the account's verified email address. The token must be invalidated immediately after first use. — Priority: Must Have
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#1)
- **FR-DOM-003** **[DOMAIN ADDED]**: Placing an order (checkout) with a newly registered account requires email verification. The customer must be given a clear, immediate way to resend the verification email without losing their cart. — Priority: Must Have
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#2)
...
- **AC-003** **[DOMAIN ENHANCED]**: Given valid login credentials, when the user submits the login form, then they are logged in within 3 seconds and redirected to the sample home page, with the nav bar showing their logged-in state. Additionally, if there have been 5 consecutive failed login attempts, further login attempts should be rejected for a cooldown period of 15 minutes.
  - *Original wording:* Given valid login credentials, when the user submits the login form, then they are logged in within 3 seconds and redirected to the sample home page, with the nav bar showing their logged-in state
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#0)
...
Enriched the SRS with account lockout after failed attempts, password reset token expiry, and email verification gating checkout based on domain knowledge.

**Knowledge sources used:**
- user_account_and_authentication.txt (3 chunk(s) used)

**Additions:**
- **FR-DOM-001** (functional_requirements): After a defined number of consecutive failed login attempts (5), the account should be temporarily locked for a cooldown period (15 minutes) or until the account owner resets their password, whichever comes first.
  - *Rationale:* Prevents brute-force password guessing attacks and improves security.
- **FR-DOM-002** (functional_requirements): When a user requests a password reset, the system issues a single-use, time-limited reset token (expiring after 30 minutes) sent to the account's verified email address. The token must be invalidated immediately after first use.
  - *Rationale:* Ensures secure and timely password recovery without security risks.
- **FR-DOM-003** (functional_requirements): Placing an order (checkout) with a newly registered account requires email verification. The customer must be given a clear, immediate way to resend the verification email without losing their cart.
  - *Rationale:* Ensures that the email address is valid and reachable for important communications.

**Modifications:**
- **AC-003** (acceptance_criteria)
  - *Before:* Given valid login credentials, when the user submits the login form, then they are logged in within 3 seconds and redirected to the sample home page, with the nav bar showing their logged-in state
  - *After:* Given valid login credentials, when the user submits the login form, then they are logged in within 3 seconds and redirected to the sample home page, with the nav bar showing their logged-in state. Additionally, if there have been 5 consecutive failed login attempts, further login attempts should be rejected for a cooldown period of 15 minutes.
  - *Rationale:* Account lockout after failed attempts is a critical security measure.
- **AC-001** (acceptance_criteria)
  - *Before:* Given a new, unused email and a valid password, when the user submits the signup form, then their account is created, their password is hashed before storage, and they are automatically logged in and redirected to the home page
  - *After:* Given a new, unused email and a valid password, when the user submits the signup form, then their account is created, their password is hashed before storage, and they are automatically logged in and redirected to the home page. Additionally, if the user's email is not verified, placing an order should be blocked until verification is completed.
  - *Rationale:* Email verification gating checkout ensures that the email address is valid and reachable.
