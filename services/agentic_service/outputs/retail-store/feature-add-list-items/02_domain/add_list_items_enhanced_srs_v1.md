...
- **[DOMAIN ADDED]** The system must support multiple currencies for price storage and display, with explicit currency codes.
- **[DOMAIN ADDED]** The system must support guest checkout, capturing a valid email address and shipping address even if the user does not create an account.
...
Enriched the SRS with additional data requirements, functional requirements, and constraints based on domain knowledge.

**Knowledge sources used:**
- checkout_and_cart_conventions.txt (2 chunk(s) used)
- product_catalog_and_inventory.txt (2 chunk(s) used)

**Additions:**
- **FR-DOM-001** (functional_requirements): Users can view the details of a specific item by clicking on it in the list.
  - *Rationale:* This allows users to see more information about an item before making a decision.
- **None** (constraints): The system must support multiple currencies for price storage and display, with explicit currency codes.
  - *Rationale:* This is necessary to handle international pricing correctly.
- **None** (constraints): The system must support guest checkout, capturing a valid email address and shipping address even if the user does not create an account.
  - *Rationale:* This is a standard e-commerce requirement to allow for flexible customer interactions.

**Modifications:**
- **AC-003** (acceptance_criteria)
  - *Before:* Given a user uploads a file that isn't an image or exceeds the size limit, when they submit, then the form shows a clear error and does not create the item.
  - *After:* Given a user uploads a file that isn't an image or exceeds the size limit, when they submit, then the form shows a clear error message specifying the issue (e.g., 'File must be in JPEG, PNG, or WebP format and not exceed 5MB.') and does not create the item.
  - *Rationale:* Clarified the error message to provide more specific feedback to the user.
