# Enhanced Software Requirements Specification: Login
...
- **FR-DOM-001** **[DOMAIN ADDED]**: The system must lock an account temporarily after five consecutive failed login attempts. — Priority: Should Have
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#0)
- **FR-DOM-002** **[DOMAIN ADDED]**: The system must invalidate a password reset token immediately after it is used once. — Priority: Should Have
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#1)
- **FR-DOM-003** **[DOMAIN ADDED]**: The system must require email verification before allowing a user to proceed with checkout. — Priority: Should Have
...
- **AC-003** **[DOMAIN ENHANCED]**: When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page, where they can enter their email address and receive a time-limited reset token via email.
  - *Original wording:* When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page.
  - *Domain source:* user_account_and_authentication.txt (user_account_and_authentication.txt#1)
...
Added a requirement for account lockout after failed login attempts based on domain knowledge. Enhanced the 'Forgot Password' acceptance criterion to clarify the reset token behavior. Also added a clarification about email verification gating checkout, which impacts user experience and security.

**Knowledge sources used:**
- user_account_and_authentication.txt (3 chunk(s) used)

**Additions:**
- **FR-DOM-001** (functional_requirements): The system must lock an account temporarily after five consecutive failed login attempts.
  - *Rationale:* This prevents brute-force attacks and protects user accounts from unauthorized access.
- **FR-DOM-002** (functional_requirements): The system must invalidate a password reset token immediately after it is used once.
  - *Rationale:* This ensures that reset tokens cannot be reused, maintaining the security of the password recovery process.
- **FR-DOM-003** (functional_requirements): The system must require email verification before allowing a user to proceed with checkout.
  - *Rationale:* This ensures that order confirmations and shipping updates can be reliably sent to a valid email address.

**Modifications:**
- **AC-003** (acceptance_criteria)
  - *Before:* When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page.
  - *After:* When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page, where they can enter their email address and receive a time-limited reset token via email.
  - *Rationale:* The enhanced description clarifies the expected behavior of the password recovery flow, including token handling and delivery.
