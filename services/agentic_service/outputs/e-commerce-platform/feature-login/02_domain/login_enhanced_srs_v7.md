...
- **FR-DOM-001** **[DOMAIN ADDED]**: The system must temporarily lock an account after five consecutive failed login attempts. — Priority: Should Have
...
- **AC-003** **[DOMAIN ENHANCED]**: When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page, where a time-limited reset token (valid for 30-60 minutes) is generated and sent to the user's verified email address. The token must be invalidated immediately upon first use and must not be reusable.
...
**Additions:**
- **FR-DOM-001** (functional_requirements): The system must temporarily lock an account after five consecutive failed login attempts.
...
- **AC-003** (acceptance_criteria)
  ...
  - *After:* When the user clicks the 'Forgot Password' link, they must be directed to the password recovery initiation page, where a time-limited reset token (valid for 30-60 minutes) is generated and sent to the user's verified email address. The token must be invalidated immediately upon first use and must not be reusable.
  - *Rationale:* This ensures secure handling of password reset flows by incorporating token expiry and reuse prevention, as per industry best practices.
