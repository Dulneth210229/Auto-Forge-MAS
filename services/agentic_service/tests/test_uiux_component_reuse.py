...
def _seed_approved_component_artifact(project_id: str, feature_id: str, component_name: str, jsx_content: str, version: int = 1):
    tmp_dir = Path(tempfile.mkdtemp())
    file_path = tmp_dir / f"{component_name.lower()}_v{version}.jsx"
    file_path.write_text(jsx_content, encoding="utf-8")
    ...
    store.artifacts[artifact_id] = {
        ...
        "artifact_format": ArtifactFormat.CODE.value,
...
def test_load_existing_approved_component_returns_verbatim_content():
    ...
    jsx = "export default function LoginForm() { return <div>login</div>; }"

    artifact_id, tmp_dir = _seed_approved_component_artifact(project_id, feature_id, "LoginForm", jsx)
...
def test_load_existing_approved_component_ignores_other_projects():
    ...
    feature_id = generate_id("feature")
    jsx = "export default function Button() { return <button />; }"

    artifact_id, tmp_dir = _seed_approved_component_artifact(other_project_id, feature_id, "Button", jsx)
...
def test_load_existing_approved_component_picks_latest_version():
    ...
    artifact_id_v1, tmp_dir_v1 = _seed_approved_component_artifact(
        project_id, feature_id, "LoginForm", "old content", version=1
    )
    artifact_id_v2, tmp_dir_v2 = _seed_approved_component_artifact(
        project_id, feature_id, "LoginForm", "new content", version=2
    )
...
def test_placeholder_mock_props_stringifies_prop_descriptions():
    agent = UIUXAgent()
    component_metadata = {
        "name": "LoginForm",
        "props": {"onSubmit": "callback invoked with credentials", "isLoading": "boolean loading state"},
    }

    result = agent._placeholder_mock_props(component_metadata)

    assert result == {
        "onSubmit": "callback invoked with credentials",
        "isLoading": "boolean loading state",
    }


def test_placeholder_mock_props_handles_missing_props():
    agent = UIUXAgent()

    assert agent._placeholder_mock_props({"name": "LoginForm"}) == {}
