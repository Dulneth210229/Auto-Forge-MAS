  ...
  return (
    <div className="max-w-3xl">
      <div className="mb-5 pb-3 border-b border-gray-200">
        <h2 className="text-lg font-bold text-gray-900">Domain Improvements</h2>
        <p className="text-xs text-gray-500 mt-1">What the Domain Agent added or changed, and why.</p>
      </div>
        ...
        <section className="mb-5">
          ...
          <div className="flex flex-wrap gap-1.5">
            {data.knowledge_sources_used.map((s) => (
              <span key={s.source_document} className="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded-full font-mono">
          ...
          <div className="flex flex-col gap-3">
            {additions.map((a, i) => (
              <div key={i} className="border border-green-200 bg-green-50 rounded-lg p-3">
                ...
                {a.rationale && <p className="text-xs text-gray-500 mt-1 italic">Why: {a.rationale}</p>}
        ...
        <section className="mb-5">
          ...
          <div className="flex flex-col gap-3">
            {modifications.map((m, i) => (
              <div key={i} className="border border-blue-200 bg-blue-50 rounded-lg p-3">
                <p className="text-xs font-semibold text-blue-800 uppercase tracking-wide">
                  {m.target_section?.replace(/_/g, " ")} {m.id && `· ${m.id}`}
                </p>
                <p className="text-sm text-gray-500 line-through mt-1">{m.original_description}</p>
                <p className="text-sm text-gray-800 mt-1">{m.enhanced_description}</p>
                {m.rationale && <p className="text-xs text-gray-500 mt-1 italic">Why: {m.rationale}</p>}
      ...
      {additions.length === 0 && modifications.length === 0 && !data.fallback_used && (
        <p className="text-sm text-gray-400 italic">No additions or modifications were recorded for this run.</p>
      )}
    </div>
  );
}
